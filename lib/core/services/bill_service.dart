...
class BillService {
  ...
  Future<ApiResponse> downloadSemesterPdfBill() async {
    try {
      final response = await dio.get(
        '/bill/export/semester/pdf',
        options: Options(
          responseType: ResponseType.bytes,
          validateStatus: (status) => status != null && status < 500,
        ),
      ...
      if (response.statusCode == 200) {
        ...
        String filename = 'monthly-bill-$date.pdf'; // A sensible default
        final contentDisposition =
            response.headers.value('content-disposition');
        if (contentDisposition != null) {
          // A simple regex to parse the filename from the header
        ...
        return ApiResponse.success("PDF Semester bill downloaded successfully!",
            filePath, response.statusCode!);
      } else {
        // Handle server-side errors like 400, 404, etc.
        return ApiResponse.error(
  ...
  Future<ApiResponse> downloadSemesterExcelBill() async {
    try {
      final response = await dio.get(
        '/bill/export/semester/excel',
        options: Options(
          responseType: ResponseType.bytes,
          validateStatus: (status) => status != null && status < 500,
        ),
      ...
      if (response.statusCode == 200) {
        // --- Get filename from headers ---
        String date = DateFormat('yyyy-MM-dd').format(DateTime.now());
        String filename = 'monthly-bill-$date.xlsx';
        final contentDisposition =
            response.headers.value('content-disposition');
        if (contentDisposition != null) {
          // A simple regex to parse the filename from the header
        ...
        return ApiResponse.success(
            "Excel Semester bill downloaded successfully!",
            filePath,
            response.statusCode!);
      } else {
        // Handle server-side errors like 400, 404, etc.
